...
contract UserSmartWalletERC20DefrostEscrowTest is Test {
    ...
    function testReturnEscrowHappyPath() public {
        vm.startPrank(admin);
        ...
        (,,,,bool isFrozen2, bool isReturned,,) = escrowContract.escrows(address(userWallet), 0);
        assertFalse(isFrozen2, "Escrow should not be marked as frozen");
        assertTrue(isReturned, "Escrow should be marked as returned");

        vm.stopPrank();
    }

    function testReturnEscrowHappyPath2() public {
        vm.startPrank(admin);
    ...
    function testReturnEscrowRevertsIfStrangerTriesToReturn() public {
        vm.startPrank(alice);
