...
contract UserSmartWalletERC20ExtendEscrowTest is Test {
    ...
    function testExtendEscrowHappyPath() public {
        vm.startPrank(admin);

        (,,uint expirationTimestamp,,,,,) = escrowContract.escrows(address(userWallet), 0);
        assertEq(expirationTimestamp, startingBlockTimestamp + escrowContract.defaultEscrowDuration());

        uint expiredTimestamp = startingBlockTimestamp + escrowContract.defaultEscrowDuration() + 1;
        vm.warp(expiredTimestamp); // Warp to after the escrow expiration
        userWallet.extendEscrow(0);

        (,,uint expirationTimestamp2,,,,,) = escrowContract.escrows(address(userWallet), 0);
        assertEq(expirationTimestamp2, expiredTimestamp + escrowContract.defaultEscrowDuration());

        vm.stopPrank();
    }

    function testExtendEscrowRevertsIfOfferHasBeenLinked() public {
        vm.startPrank(admin);
    ...
    function testExtendEscrowRevertsIfEscrowHasNotExpired() public {
        vm.startPrank(admin);

        uint notYetExpiredTimestamp = startingBlockTimestamp + escrowContract.defaultEscrowDuration() - 1;
        vm.warp(notYetExpiredTimestamp); // Warp to before the escrow expiration
